...
function onResize(event: MouseEvent) {
    ...
    if (state.is_mouse_down && state.is_resizing && state.selected_div && draggableDiv && chartDiv) {
        ...
        // Apply table-specific minimum dimensions
        if (state.selected_chart && state.selected_chart.chart_type === 'table') {
            // Calculate minimum width based on columns
            const columnCount = state.selected_chart.columns?.length || 0;
            const minColumnWidth = 120; // From component prop
            const rowNumberWidth = 60; // If row numbers enabled
            const padding = 80; // Container padding and borders
            const calculatedMinWidth = (columnCount * minColumnWidth) + rowNumberWidth + padding;
            
            const finalMinWidth = Math.max(calculatedMinWidth, 400);
            
            newWidth = Math.max(finalMinWidth, newWidth);
            newHeight = Math.max(250, newHeight);
            newWidthDraggable = Math.max(finalMinWidth, newWidthDraggable);
            newHeightDraggable = Math.max(250, newHeightDraggable);
        }
...
async function handleTemplateSelect(widgets: any[], dataModelId: number | null, templateName: string) {
    state.show_template_selector = false;

    // Server-side template clone flow
    if (templateName.startsWith('server:')) {
        const serverTemplateId = parseInt(templateName.replace('server:', ''));
        try {
            const token = getAuthToken();
            const config = useRuntimeConfig();
            const result = await $fetch<{ message: string; dashboard: any }>(
                `${config.public.apiBase}/dashboard/clone-template`,
                {
                    ...
                    body: JSON.stringify({ template_id: serverTemplateId, project_id: projectId.value }),
                },
            );
            if (result?.dashboard?.id) {
                ...
                router.push(`/projects/${route.params.projectid}/dashboards/${result.dashboard.id}`);
            }
        } catch (err: any) {
    ...
    // Local template flow — populate canvas with widgets
    state.dashboard.charts = [];
    widgets.forEach((widget) => {
        state.dashboard.charts.push(widget);
    });
    if (import.meta.client) {
        nextTick(() => {
            state.dashboard.charts.forEach((chart: any) => {
...
<template>
    <DashboardTemplateSelector
        v-if="state.show_template_selector"
        :data-models="state.data_model_tables.map((dm: any) => ({
            data_model_id: dm.data_model_id,
            table_name: dm.model_name,
            logical_name: dm.logical_name,
            columns: dm.columns || [],
        }))"
        :project-id="projectId"
        @select="handleTemplateSelect"
        @skip="handleSkipTemplate"
        @close="handleSkipTemplate"
    />
                        ...
                        >
                            <div class="flex flex-col bg-white rounded-lg shadow-md">
                                ...
                                >
                                    ...
                                    />
                                    <font-awesome
                                        v-if="isDataModelDataEnabled(chart.chart_id)"
                                        icon="fas fa-table"
                                ...
                                <div v-if="chart.chart_type === 'text_block'" :id="`draggable-${chart.chart_id}`" class="bg-gray-200 border border-3 border-gray-600 border-t-0">
