...
function createSingleSource(dataSourceId: number) {
  router.push(`/marketing-projects/${projectId.value}/data-sources/${dataSourceId}/data-models/create`);
}

function createCrossSource() {
  router.push(`/marketing-projects/${projectId.value}/data-models/create`);
}
...
function viewModel(model: any) {
  ...
  if (model.is_cross_source) {
    ...
    router.push(`/marketing-projects/${projectId.value}/data-models/${model.id}/edit`);
  } else {
    // Single-source model - navigate to data source specific details page
    const dataSourceId = model.data_source?.id;
    if (dataSourceId) {
      router.push(`/marketing-projects/${projectId.value}/data-sources/${dataSourceId}/data-models/${model.id}/edit`);
    } else {
      $swal.fire({
        icon: 'error',
        title: 'Error',
