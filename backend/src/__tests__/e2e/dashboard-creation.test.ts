...
describe('E2E: Dashboard Creation Flow', () => {
    ...
    beforeAll(async () => {
        ...
        const orgResult = await concreteDriver.manager.query(
            ...
            [`Dashboard E2E Org ${Date.now()}`, userId, new Date()]
        );
        ...
        const wsResult = await concreteDriver.manager.query(
            ...
            [`Dashboard E2E Workspace ${Date.now()}`, organizationId, new Date()]
        );
        ...
        const projResult = await concreteDriver.manager.query(
            ...
            [`Dashboard E2E Project ${Date.now()}`, userId, organizationId, workspaceId, new Date()]
        );
    ...
    afterAll(async () => {
        try {
            const concreteDriver = await dbDriver.getConcreteDriver();
            // Cascade deletes will handle workspaces, projects, data sources, models, dashboards
