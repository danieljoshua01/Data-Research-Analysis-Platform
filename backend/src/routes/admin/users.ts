...
// Get enterprise inquiry for conversion
router.get('/convert/:betaUserId', async (req: Request, res: Response, next: any) => {
    ...
    const betaUser = await UserManagementProcessor.getInstance().getEnterpriseQueryForConversion(req.body.tokenDetails, betaUserId);
    if (betaUser) {
        res.status(200).send(betaUser);
    } else {
        res.status(404).send({ message: 'Enterprise inquiry not found or already converted' });
    }
});
