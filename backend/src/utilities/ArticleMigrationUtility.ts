 ...
 * Articles are converted from HTML to markdown on demand.
 */
...
export class ArticleMigrationUtility {
    ...
    async articleNeedsMigration(articleId: number): Promise<boolean> {
        ...
        return article ? (!article.content_markdown || article.content_markdown.trim() === '') : false;
    }
    
    /**
     * Convert a single article to markdown
     */
    async convertArticleToMarkdown(articleId: number, tokenDetails: ITokenDetails): Promise<boolean> {
    ...
    /**
     * Batch convert all articles without markdown for a specific user
     */
    ...
    /**
     * Get migration status for all articles
     */
    async getMigrationStatus(): Promise<{
        totalArticles: number;
        withMarkdown: number;
        withoutMarkdown: number;
        ...
        const totalArticles = await manager.count(DRAArticle);
        
        const withMarkdown = await manager
            .createQueryBuilder(DRAArticle, 'article')
            .where('article.content_markdown IS NOT NULL')
            .andWhere('article.content_markdown != :empty', { empty: '' })
            .getCount();
        
        const withoutMarkdown = totalArticles - withMarkdown;
        const migrationProgress = totalArticles > 0 ? (withMarkdown / totalArticles) * 100 : 0;
