    ...
    async getPublicArticles(): Promise<IArticle[]> {
        return new Promise<IArticle[]>(async (resolve, reject) => {
            ...
            for (let i = 0; i < articles.length; i++) {
                ...
                // Create a shallow copy with the extra field to avoid modifying the entity directly if needed,
                // but since IArticle is just an interface wrapping the entity, we can just attach it.
                // Or better, add an 'updated_at' property to IArticleData or IArticle.
                // Assuming IArticleData is what the frontend expects, let's look at IArticleData.
