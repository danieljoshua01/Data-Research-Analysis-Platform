...
async function runMigrations() {
  ...
  try {
    // Override migration DB host/port to use Docker network hostname
    const { stdout, stderr } = await execa('docker', [
      'exec',
      '-e', 'POSTGRESQL_HOST_MIGRATIONS=database.dataresearchanalysis.test',
      '-e', 'POSTGRESQL_PORT_MIGRATIONS=5432',
...
async function runSeeders() {
  ...
  try {
    // Override seeder DB host/port to use Docker network hostname
    const { stdout, stderr } = await execa('docker', [
      'exec',
      '-e', 'POSTGRESQL_HOST_MIGRATIONS=database.dataresearchanalysis.test',
      '-e', 'POSTGRESQL_PORT_MIGRATIONS=5432',
