...
/**
 * Check if PostgreSQL inside its container is ready to accept connections
 */
async function checkPostgresReady(containerName) {
  try {
...
/**
 * Check if Redis inside its container is ready to accept connections
 */
async function checkRedisReady(containerName) {
  try {
    const { stdout, exitCode } = await execa('docker', [
      'exec', containerName, 'redis-cli', 'ping'
    ], { reject: false });
    return exitCode === 0 && stdout.trim() === 'PONG';
  } catch {
    return false;
  }
}
...
async function waitForServicesHealthy() {
  ...
  const services = [
    {
      name: 'database.dataresearchanalysis.test',
      displayName: 'PostgreSQL Database',
      readyCheck: async (name) => {
        if (!await checkContainerRunning(name)) return false;
        return checkPostgresReady(name);
      }
    },
    {
      name: 'redis.dataresearchanalysis.test',
      displayName: 'Redis',
      readyCheck: async (name) => {
        if (!await checkContainerRunning(name)) return false;
        return checkRedisReady(name);
      }
    },
    {
      name: 'backend.dataresearchanalysis.test',
      displayName: 'Backend API',
      readyCheck: checkContainerRunning
    },
    {
      name: 'frontend.dataresearchanalysis.test',
  ...
  for (const service of services) {
    ...
    try {
      await pRetry(
        async () => {
          const ready = await service.readyCheck(service.name);
          if (!ready) {
            throw new Error(`${service.name} not ready yet`);
          }
        },
        {
          retries: 60,
          ...
          maxTimeout: 4000,
          onFailedAttempt: (err) => {
            spinner.text = `Waiting for ${service.displayName} (attempt ${err.attemptNumber}/60)...`;
          }
        }
      );
